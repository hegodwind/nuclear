...
class AnalyticalSolver:
    """
    基于线性链解析解 (Linear Chain Method / TTA) 的求解器。
    1. 从燃耗矩阵 A 中自动搜索所有可能的反应链 (DFS)。
    2. 对每一条链应用 Bateman 方程求解。
    3. 采用 微扰法)处理重复特征值问题。
    """

    def __init__(self, cutoff=1e-12, max_depth=10):
        """
        :param cutoff: 截断阈值。当某条链的累积通过率(Passage)小于此值时停止追踪。
        :param max_depth: 最大搜索深度，防止循环链导致的无限递归。
        """
        self.cutoff = cutoff
    ...
    def solve(self, A, N0, dt):
        """
        解析法求解接口
        :param A: 燃耗矩阵 (scipy.sparse.csr_matrix)
        :param N0: 初始浓度向量
        :param dt: 时间步长
        """
        # 1. 准备数据
        size = A.shape[0]
        N_next = np.zeros(size)
        ...
        # 2. 遍历所有初始浓度不为 0 的核素作为链的起点
        sources = np.where(N0 > 0)[0]
        
        for root_idx in sources:
            root_conc = N0[root_idx]
            # 开始递归搜索链: [root]
            self._dfs_chains(
                current_idx=root_idx,
                chain_nodes=[root_idx], 
                chain_lambdas=[lambda_effs[root_idx]], 
                branching_ratios=[1.0], 
                time=dt,
                start_conc=root_conc,
                N_accum=N_next,
                A_csc=A_csc,
                all_lambdas=lambda_effs
            )
            
        return N_next

    def _dfs_chains(self, current_idx, chain_nodes, chain_lambdas, branching_ratios, 
                   time, start_conc, N_accum, A_csc, all_lambdas):
        """
        深度优先搜索构建线性链，并实时计算贡献
        """
        path_contribution = self._bateman_equation(start_conc, chain_lambdas, branching_ratios, time)
        
        # 累加到结果向量
        N_accum[current_idx] += path_contribution
        
        # 2. 检查停止条件
        if path_contribution < self.cutoff * start_conc and len(chain_nodes) > 1:
            return
        if len(chain_nodes) >= self.max_depth:
            return

        # 3. 寻找
        col_start = A_csc.indptr[current_idx]
        col_end = A_csc.indptr[current_idx+1]
        
        children_indices = A_csc.indices[col_start:col_end]
        transfer_rates = A_csc.data[col_start:col_end]
        
        for child_idx, rate in zip(children_indices, transfer_rates):
            # 排除对角线元素 
            ...
            self._dfs_chains(
                ...
                chain_nodes=chain_nodes + [child_idx],
                chain_lambdas=chain_lambdas + [all_lambdas[child_idx]],
                branching_ratios=branching_ratios + [rate],
                time=time,
    ...
    def _bateman_equation(self, N0, lambdas, transfer_rates, t):
        """
        通用 Bateman 方程求解 (包含微扰法处理重根)
        :param N0: 链首初始浓度
        :param lambdas: 链上各核素的有效衰变常数 [lam1, lam2, ..., lam_n]
        :param transfer_rates: 链上的转移速率 [1.0, k_1->2, k_2->3, ...] (长度与lambdas一致)
                               注意: 第一个是占位符或1.0，实际转移从索引1开始
        :param t: 时间
        """
        n = len(lambdas)
        # 处理重根问题: 微扰法引入微小差异
        ...
        for i in range(n):
            for j in range(i + 1, n):
                while abs(lams[i] - lams[j]) < 1e-10 * abs(lams[i]):
                    lams[j] *= 1.0001 
        
        # 1. 计算前置系数 (Product of Transfer Rates)
        prod_k = 1.0
        for k in range(1, n):
            prod_k *= transfer_rates[k]
            
        if prod_k == 0: return 0.0

        # 2. 计算求和项 (Sum of Exponentials)
        sum_val = 0.0
        for i in range(n):
            # 计算分母: Prod_{j!=i} (lam_j - lam_i)
            denom = 1.0
            for j in range(n):
                if i != j:
                    denom *= (lams[j] - lams[i])
            ...
            # 累加
            if denom != 0:
                sum_val += (np.exp(-lams[i] * t) / denom)
        
        return N0 * prod_k * sum_val
    ...
    def solve(self, A, N0, dt, steps=1):
        ...
        for s in range(steps):
            if self.method == 'scipy_expm':
                ...
                current_N = analytical_solver.solve(A, current_N, dt_sub)
...
with st.sidebar:
    ...
    solver_option = st.selectbox(
        "数值积分方法", 
        ["Matrix Exponential (Scipy)", "ORIGEN Method (泰勒展开 + 平衡近似)","CRAM Method (切比雪夫有理逼近法)", "Analytical Solver (线性链解析解)"]
    )
    # 映射选项到内部方法名
    if "Matrix Exponential" in solver_option:
        ...
        solver_method = 'cram'
    elif "Analytical Solver" in solver_option:
